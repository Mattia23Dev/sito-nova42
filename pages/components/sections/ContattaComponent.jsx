...
const ContattaComponent = ({bg=''})=> {
    ...
    useEffect(()=>{ 
        ...
        if(newsLetter){
            setNewsLetterMessage("<p><strong>ACCONSENTO</strong> all'invio di materiale infotmativo via e-mail da parte di Mattia Noris riguardo le ultime notizie e suoi nuovi servizi.</p>")
        }else{
            setNewsLetterMessage("<p><strong>NON ACCONSENTO</strong> all'invio di materiale infotmativo via e-mail da parte di Mattia Noris.</p>")
        }

        if(isValidEmail && isValidPhone && email.length && name.length && company.length && message.length){
            setDisabled(false);
        }else{
            setDisabled(true);
        }

    },[email,name,company,phone,message,newsLetter,disabled,isValidEmail, isValidPhone]);
    ...
    const sendEmail = ()=> {    
        ...
        const summaryEmail = {
            from: email,
            subject: `RICHIESTA DI CONTATTATO DA ${company}`,
            message: `<p>
                        Ciao <strong>Noris Mattia</strong>,<br/> 
    ...
    return (
        <div className={bg}>
            <div className="form-group mt-5">
                ...
                <input type="text" className="form-control mt-2" value={name} onChange={(e)=>setName(e.target.value)} placeholder="Nome e comgnome" />
            </div>
            <div className="form-group mt-3">
                <label className="text-white text-capitalize" htmlFor="formGroupExampleInput">Azienda</label>
                <input type="text" className="form-control mt-2" value={company} onChange={(e)=>setCompany(e.target.value)} placeholder="Nome azienda" />
            </div>
            <div className="form-row row">
                <div className="col-12 col-md-6">
                    <div className="form-group mt-3">
                        <label className="text-white text-capitalize" htmlFor="formGroupExampleInput">E-mail aziendale</label>
            ...
            <div className="row mt-4 pb-5">
                <div className="form-group">
                    <div className="form-check">
                        ...
                        <p className="text-white size-12 mt-2">Se cambi idea puoi disinscriverti <strong className="text-purple">quando vuoi</strong> e accetto le sue
                        <Link href={'/privacy'}><a style={{color: 'blue'}}> Privacy policy</a></Link>.</p>
                    </div>
                </div>
                <div className="d-flex justify-content-center align-content-center">
                    ...
                    <p className="text-muted size-12 mt-3 p-4 text-center">Inviando questo form acconsento ad essere contattato dal team di Nova42.</p>
            </div>
        </div>
    )
...
export default ContattaComponent